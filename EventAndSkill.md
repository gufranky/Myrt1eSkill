...
- **类型**: 被动技能
- **特殊机制**:
  - 监听武器开火事件（EventWeaponFire）
  - 每个玩家随机分配 20%-30% 概率
  - **使用 CS2TraceRay 库进行真实的射线追踪**
  - 检测墙壁和玩家碰撞，在击中点爆炸
  - 跳过射击者自己的实体
  - 爆炸伤害 25，半径 210
  - 使用特殊角度识别自己创建的爆炸
  - 如果射线未击中任何物体，使用最大射程（2000单位）
...
- **特殊机制**:
  ...
  - 死亡时在玩家位置创建 HE 手雷抛射物
  - 修改手雷属性：伤害 999，爆炸半径 500
  - 手雷立即爆炸（DetonateTime = 0）
  - 特殊角度标识（10, -5, 9）用于识别穆罕默德手雷
  - 参考 jRandomSkills Muhammed 技能实现
  - GPLv3 许可：代码和设计概念来自 jRandomSkills
