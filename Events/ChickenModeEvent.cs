...
// Based on jRandomSkills by Juzlus (ChickenModel skill)
...
public class ChickenModeEvent : EntertainmentEvent
{
    ...
    // 记录隐藏实体索引（用于 CheckTransmit）
    private readonly Dictionary<ulong, HashSet<uint>> _invisibleEntities = new();
    ...
    private void ApplyChickenEffects(CCSPlayerController player)
    {
        ...
        // 记录需要隐藏的实体（武器、手套等）
        RecordInvisibleEntities(player, true);
    ...
    private void RemoveChickenEffects(CCSPlayerController player)
    {
        ...
        // 清除隐藏实体记录
        RecordInvisibleEntities(player, false);
